...
**調査層から規範層への参照を欠かしてはなりません**。参照がないと、規範層の改訂が調査層へ伝わらず、同じ体系の中で矛盾した記述が並びます。とくに調査層は作成した時点の技術的な前提に依存するため、参照を持たない記述は規定より先に古くなります。
...
| AC(前提の充足状態) と CR(受入基準) | 別の体系。AC は前提が成立しているか、CR は個別の機能が満たすべき条件 |
| SC(スコープ3層) と R(リスク区分) | SC は範囲の確約度、R は決定の取り消し可能性。**R1 の変更が調整枠(SC-B)に属することもある** |

**新しい区分を追加する場合、必ず本表を更新してください**。記号が重複すると、別の概念が同じ記号で書かれ、後から改称が必要になります。改称は参照するすべての箇所へ波及します。
...
| ライセンス検出の精度、および複製検知の検出率を測った独立の評価 | 特定できていない。本標準はどちらの数値も引用していない |
| 最高裁 平成22年7月15日判決(経営判断の原則)の原文 | 未取得。解説記事の引用による。**規制対応の文脈で引用する前に原本の確認が必要** |
| コードレビューにおける手がかりの提示時期を扱った実験 | 特定できていない。[レビューパッケージ](/process-compass/phase5-implementation/review-package/)の提示順序は別分野の知見からの類推 |
