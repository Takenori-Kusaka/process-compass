...
規則の競合(例: PoC は独立レビューを省略、規制業は厳格化)に備えて `priority` を持たせています(補助の限界ケース 40 > 品質・規制 30 > 開発形態 20 > 規模・事業フェーズ 10)。適用順序と競合解決の意味論は[提案ロジック設計](/process-compass/tool/proposal-logic/)で定義します。
