...
- 提案のベースは[ピットイン方式参照モデル](/process-compass/phase4-process-design/process-model/)
...
- 質問7で「利用不可」の場合も従来型プロセスの整理として提案を返す(AIなしでもピットイン方式の外殻は機能する)
...
| 実装 | 内容 | このページとの関係 |
| --- | --- | --- |
| 知識ベーススキーマ設計 | 入力項目と規則を機械可読にする | 調整軸・規則の型定義 |
| 提案ロジック設計 | 規則の適用順序・競合解決を定義する | 判定の順序 |
| UI プロトタイプ | 入力フォームと出力表示を実装する | 画面の要件 |
| 修正・採択ワークフロー | 上書き・警告・URL 共有・提案書出力を実装する | 出力の要件 |
| ユーザーテスト | 利用シナリオの3ペルソナ相当で検証する | 受け入れの観点 |
