...
| EV-0033 | 決定1(例外承認の経路を置けば、較正のデータが供給されるという判断) | E0(なし) | 案件 P-001。7.3 の適用実績は0件であり、例外承認が実際に使われるかを支持する観測を持ちません | 2027-02 | 四半期ごとに次を数える。(a) 上限超過により差し戻した件数、(b) 例外承認を経て超過のままレビューを実施した件数、(c) (b) のうち判定完了時間が SLA を満たした件数。**(a) > 0 かつ (b) = 0 が2四半期続いた場合、例外承認の経路が回避されている**。その場合は上限を合否条件から外し、目安のみとする | [adr/0047-limit-is-default-not-cutoff](/process-compass/adr/0047-limit-is-default-not-cutoff/) |
| EV-0034 | 決定2(本文の表から機械抽出すれば、引き写しの誤りが無くなるという判断) | E2(単一事例) | 案件 P-001。判定へ基準外の事項を持ち込んだ2件の観測に基づきます | 2027-02 | 判定基準の一覧を配布した後に、**基準外の事項を判定へ持ち込んだ件数**。配布後も件数が減らない場合、原因は実物の不在ではなく判定の範囲の理解だったことになります | [adr/0048-gate-criteria-source-of-truth](/process-compass/adr/0048-gate-criteria-source-of-truth/) |
| EV-0035 | 決定2(語彙を閉じた集合に保てば、誤検出の総量が予測可能に保たれるという判断) | E0(なし) | 2026-08-14 の語彙拡張1回分 | 2027-02 | 語彙を拡張した回ごとの、注記へ移した件数と除外へ登録した件数の比。**除外が注記の5倍を超える拡張が2回続いた場合、語彙の選び方が限定の検出に向いていないことになります**。初回は 12対0 であり、既にこの水準を超えています | [adr/0049-scope-units-and-closed-vocabulary](/process-compass/adr/0049-scope-units-and-closed-vocabulary/) |
| EV-0038 | 決定1(配布物の件数を検査すれば、配送の欠落を検出できるという判断) | E2(単一事例) | 2026-08-14 の判定基準の消失1件 | 2027-02 | 配布項目ごとに、**件数は一致するものの中身が異なる**事象を数える。件数の一致だけで通過した誤配が1件でも生じた場合、件数の検査では足りず、内容の突合が要ることになります | [adr/0052-delivery-is-verified-at-the-destination](/process-compass/adr/0052-delivery-is-verified-at-the-destination/) |
| EV-0039 | 決定1・2(基準で定義し列挙を例示とすれば、次の漏れを防げるという判断) | E1(間接) | ADR-0038 決定1(統制の弱化を性質で定義した先例)。**同 ADR の適用実績は、本件が最初です** | 2027-02 | 次の2つを数える。(a) 例示へ載せていない操作が、基準を満たすものとして停止された件数。(b) 基準を満たすのに停止されなかった件数。**(b) が1件でも生じた場合、基準の記述は実行主体が判定できる形になっていません** | [adr/0053-irreversible-by-criterion-not-enumeration](/process-compass/adr/0053-irreversible-by-criterion-not-enumeration/) |
...
| EV-0019 | 投影の8項目の構成、G.3 の提示の順序、および G.6 の形式の作法 | E0(なし) | 案件 P-001 の決裁1件(資料4版・棄却3回)。**ただし当該案件は投影を用いていません**。附属書G が新設される前に生成された構成で運用していたためです | 2027-08 | **投影を用いた**決裁3件分について次を記録する。(a) 1件の決裁を通すまでに作成した資料の版数、(b) 各棄却の理由が G.2 の8項目のいずれに対応するか、(c) 8項目のいずれにも対応しない棄却の件数、(d) 8項目に対応するが投影が実施されていなかったことによる棄却の件数 | [phase4-process-design/executive-projection](/process-compass/phase4-process-design/executive-projection/) |
...
| EV-0036 | 決定3(立ち上げ期の過剰な統制を、3つの原因へ切り分ければ扱えるという判断) | E2(単一事例) | 案件 P-001 の3事象。**うち2件はテンプレートから作った直後の一度きりの期間に生じており、2件目以降の案件では再現しません** | 2027-08 | 2件目以降の案件で、立ち上げ期に強制層が実務を止めた件数と、その原因の内訳。**3つのいずれにも当てはまらない事象が生じた場合、ステージによる緩和の必要性を再検討します** | [adr/0050-enforcement-layer-is-not-staged](/process-compass/adr/0050-enforcement-layer-is-not-staged/) |
| EV-0037 | 決定2(価値と受入基準の対応を見れば、価値の取りこぼしを検出できるという判断) | E2(単一事例) | 案件 P-001。企画書に価値として書いた項目のうち、受入基準へ対応しないものが存在した1件の観測に基づきます | 2027-08 | G-2 の確認で「対応する受入基準が無い価値」が指摘された件数と、そのうち後工程で価値の位置づけを見直した件数。**指摘が0件のまま、後工程で価値の位置づけの見直しが生じた場合、この確認は取りこぼしを検出していないことになります** | [adr/0051-usability-out-of-scope](/process-compass/adr/0051-usability-out-of-scope/) |
