...
プロセスは3つのズームレベルで整理し、図のノードをクリックすると下位レベルのページへ移動できるようにします(表現ポリシーは [ADR-0006](/process-compass/adr/0006-representation-policy/))。1図あたりのノードは15個前後を上限とし、超える場合は分割します。

| ズームレベル | 内容 | 図解様式 |
| --- | --- | --- |
| L1: 全体マップ | フェーズの流れとゲート配置 | `graph LR` + ゲートは菱形ノード + click で L2 へ |
| L2: フェーズ内ワークフロー | アクティビティの連なりとロールの関与 | `sequenceDiagram`(ロール間)or `graph LR` + click で L3 へ |
| L3: 個別作業 | タスクの手順・判断分岐 | `flowchart` |

各レベルのページは「図(構造)+ 平易な解説(なぜ・何のため)+ 原典対照(出典付き引用)」の3点セットで構成します。難解な原典定義は「平易な言い換え → 原典引用」の対で示します。

なお、プロセスの公開データは共通 YAML スキーマから生成する方針です([ADR-0007](/process-compass/adr/0007-schema-driven-process-data/)。スキーマ v0 はウォーターフォール+スクラムの2プロセスで検証してから展開)。
