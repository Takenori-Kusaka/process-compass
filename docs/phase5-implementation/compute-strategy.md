...
ピットイン方式では、機械検査(G-5)とAIエージェントが従来より桁違いに多く実行されます。全部をクラウド CI に載せるとコストと待ち時間が破綻するため、**検査の実行場所を多段化**する戦略を取ります。
...
上の予算管理は消費の**総量**を扱います。暴走そのものを止めるには、実行時に遮断する仕組みが別に必要です([第3章 3.12.10](/process-compass/phase4-process-design/roles-responsibilities/))。

### どこで強制するか

```mermaid
graph TD
  A["エージェント実行基盤<br>ループ内の反復"] --> D{"停止条件"}
  B["接続経路(プロキシ・ゲートウェイ)<br>タスク単位の累積"] --> D
  C["CI ジョブ<br>タイムアウト"] --> D
  D -->|発火| E["実行を停止し記録<br>人へ返す"]
```

| 層 | 強制できる条件 | 実装 |
| --- | --- | --- |
| エージェント実行基盤 | 連続失敗回数・無進捗 | エージェントのループ制御。反復ごとに判定する |
| 接続経路 | 単一タスクの累積トークン量・金額 | プロキシまたはゲートウェイで、タスク識別子ごとに積算する |
| CI ジョブ | 実行時間 | 全ジョブの明示的なタイムアウト(前掲) |

**エージェント自身の指示文で停止を実現しません**。指示は守られないことがあります。停止は実行基盤か接続経路の側、つまりエージェントが回避できない層で強制します。

### 無進捗をどう判定するか

「同じ失敗を繰り返している」ことの機械的な判定です。実装の候補を挙げます。

- 直前の反復と同一の検査が、同一の識別子(テスト名・ルール名)で失敗している
- 生成された差分が、直前の状態へ戻っている(A → B → A の往復)
- 検査の失敗件数が、閾値の反復にわたって減っていない

**最も安価なのは1つ目です**。テストランナーや lint の出力から失敗した項目の識別子を取り出し、前回と突き合わせるだけで済みます。まずこれから始めて、振動を取りこぼすようなら2つ目を足します。

### 記録する項目

停止は失敗として記録します。記録がないと閾値を較正できません。

| 項目 | 用途 |
| --- | --- |
| 発火した条件 | どの条件が効いているかの把握 |
| 反復回数と消費量(トークン・金額) | 閾値の較正 |
| 最後の失敗内容 | 正当な実行を止めたかどうかの判定 |
| タスク識別子 | 同一タスクでの再発の検知 |

### 誤検知への対処

正当な実行を止めた場合の再開は、**人が明示的に行います**。自動での再開は、遮断そのものを無効にします。

停止記録のうち、正当な実行を止めた割合を月次で集計します。この割合が高い場合、閾値ではなく**タスクの与え方**を疑います。1回の実行へ渡す範囲が広いほど、失敗の内容は安定せず、無進捗の判定もすり抜けます。
