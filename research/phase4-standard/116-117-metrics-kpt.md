...
出典:
- Jeffries の発言をまとめた二次情報: https://www.linkedin.com/posts/abinoda_measuring-the-impact-of-ai-coding-assistants-activity-7161005023978475520-KHUW （※直接の一次ソースは Jeffries 本人の X 投稿およびブログ。**一次 URL 未確認**）
...
- クラスタ分析により**7つのチーム類型**を提示（例: Foundational Challenges＝低性能・高不安定・高バーンアウト、Harmonious High Achievers＝多面的に高성과）
...
- 「なぜ○○さんは」という主語を**「どの仕組みが」に置換**するルールを明文化する
...
- **シニアリーダーシップの率先垣範**。経営層がレビューに参加し、創業者が全社フォーラムで優れたインシデント対応を称賛する
...
**未確認 / 追加調査が必要**: 「レトロスペクティブの改善アクション実施率」に関する定量的な実証研究（例: Andriyani, Lehtinen らの研究）は、Web 検索の予算上限により本調査では取得できませんでした。#117 の設計根拠として重要なため、追加調査を推奨します。
...
- **個人攻撃化する**: 「なぜあの人はミスをしたのか」という詰問になり、パワーハラスメント化する恐れがある
...
- なぜなぜ分析の「なぜ」という問いは、日本語では主語を人に取りやすい。「なぜ○○さんは」ではなく「**どの仕組みが**」へ言い換えるルールの明文化が、実務上の最小の対策になる
