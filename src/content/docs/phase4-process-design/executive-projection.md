...
**本節は形式の作法であり、投影へ新しい項目を加えるものではありません**([ADR-0032](/process-compass/adr/0032-executive-summary-as-projection/)決定2)。いずれも G.2 の8項目の**出し方**を述べます。

| # | 作法 | 出所 |
| --- | --- | --- |
| 1 | 冒頭に、**何を承認してほしいかを1行で置く** | D-0 表1 決定の権限 |
| 2 | 「誰の何が変わるか」を、**受益者ごとに分けて提示する** | 企画書 観点3 |
| 3 | 構造を持つ項目(体制・費用の区分・工程)は**図で提示する**。文字の羅列で置き換えない | D-0 体制図、第7章 7.7.1 の4区分 |
| 4 | **プロダクトを一言で表す1文**を、観点3 の記述から取り出して冒頭に置く | 企画書 観点3 |

**作法3の理由は、AI 協調型開発に固有です**。成果物の多くがテキストで生成されるため、その形式のまま決裁へ持ち込みやすくなります。**構造を持つ項目を文字の羅列で提示すると、決裁者は構造を自分で復元することになります**。

**本節は提示の形式を指定するものであり、遵守を求める範囲を超えます**(G.1 の宣言のとおり)。**決裁制度の様式は組織ごとに異なります**。

:::caution[暫定 EV-0019 / 見直し 2027-08]
**対象**: 投影の8項目の構成、G.3 の提示の順序、および G.6 の形式の作法。

**根拠の水準**: E0(なし)。

**根拠とした事例**: 案件 P-001 の決裁1件(資料4版・棄却3回)。**ただし当該案件は投影を用いていません**。附属書G が新設される前に生成された構成で運用していたためです。

**差し替え条件**: **投影を用いた**決裁3件分について次を記録する。(a) 1件の決裁を通すまでに作成した資料の版数、(b) 各棄却の理由が G.2 の8項目のいずれに対応するか、(c) 8項目のいずれにも対応しない棄却の件数、(d) 8項目に対応するが投影が実施されていなかったことによる棄却の件数。

**判定**:

- **(c) が棄却総数の過半を占めるとき** — 8項目の構成が不足している。項目を見直す
- **(d) が過半を占めるとき** — 構成は足りている。不足しているのは附属書G の配送である
