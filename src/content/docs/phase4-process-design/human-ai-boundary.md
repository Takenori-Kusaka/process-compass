...
**上表の4種類は、いずれも人間が担います**。担当ロールはすべて人間のロールであり、AI へ割り当てられません。AI は、判断の材料を提示する実行(R)を担います。

5.8.3 との違いは、**禁止の理由**です。5.8.3 の4項目は、AI へ担わせると**安全または検証の成立そのものが崩れる**ため禁じます。本節の4種類は、**結果責任(A)を負う者による判断でなければ責任の所在を失う**ため人間が担います。**どちらも自律レベルによって変わりません**。

本節を**初期値**や**推奨**として読まないでください。案件が判断記録(ADR)を書けば AI へ委譲できる、という規定ではありません。役割境界の見直しは 5.9 によります。**見直しの結果は本章へ反映するものであり、案件側の記録で本章の要求を緩和する経路はありません**。

<!-- impl IMPL-0019 target=claude-md state=delivered note="人間が担う判断の4種類を、AI へ割り当てられないものとして実行層へ書く" -->
