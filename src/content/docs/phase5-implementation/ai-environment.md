...
| 層 | 形態 | 統制 | 導入順 |
| --- | --- | --- | --- |
| E1 対話支援 | IDE 補完・チャット | 人が全出力を直接確認 | 最初 |
| E2 エージェント(監督付き) | CLI エージェント(ローカルで人が監督) | 権限設定+人の承認 | 中核 |
| E3 自律エージェント | CI・クラウド上で非同期実行 | 全ゲート機械強制+独立レビュー必須 | 最後 |

E3(自律)へ進むほど、[Git 戦略](/process-compass/phase5-implementation/git-strategy/)のルールセットと [CI ゲート](/process-compass/phase5-implementation/ci-gates/)が生命線になります。**ゲートの機械強制ができていない段階で E3 を解禁しない**ことが原則です。

:::caution
本ページの E1〜E3 は**実行形態**(どこで、どのように動かすか)の区分です。[第5章 5.4](/process-compass/phase4-process-design/human-ai-boundary/)の **AI自律レベル L1〜L3** とは軸が異なります。混同しないでください。

| 観点 | 実行形態(E1〜E3) | AI自律レベル(L1〜L3) |
| --- | --- | --- |
| 何を区分するか | 実行の場所と監督の手段 | 監視者・フォールバックの担い手・適用範囲 |
| 決めるもの | 権限設定と機械的強制の要否 | エージェントが動作してよい領域の広さ |
| 規定する場所 | 本ページ(フェーズ5) | 第5章(フェーズ4) |

両者は独立に設定します。E3(CI 上での非同期実行)であっても、対象がリスク区分 R1 であれば AI自律レベルは L1 に固定されます。
:::
...
```mermaid
graph LR
  A["E1 対話支援<br>(全員・即日)"] --> B["E2 監督付きエージェント<br>(権限設定+フック整備後)"]
  B --> C["E3 自律エージェント<br>(ルールセット+CIゲート稼働後)"]
```
