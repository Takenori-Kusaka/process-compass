...
**本標準は、他組織がそれに従って開発するための規格である**。IPA や規格策定機関の立場に立つ。寄せられたフィードバックをそのまま条項にすることは、この立場と両立しない。

- **YOU MUST**: Issue・レビュー・申し送り・外部AIの提案を、**そのまま標準へ反映しない**。起票者の提案が正しいという前提を置かない
- **YOU MUST**: 標準の条項を新設・改廃する前に、次の3つを行う
  1. **調査** — サブエージェントを並列起動し、既存規格(ISO/IEC/IEEE、JIS、IPA、SWEBOK 等)と業界の現在の実務を調べる。「世の中がどうなっているか」を踏まえない条項は書かない
  2. **議論** — 複数の立場を持つエージェントで**会議体を開き**、対立する見解を突き合わせる。単一の案を磨くのではなく、案を競わせて落とす
  3. **突合** — 既存の ADR・条項・語彙と矛盾しないかを検査する。矛盾する場合は、どちらが誤りかを決める
- **最小限の変更という動機は存在しない**。安さ・変更量の少なさを採否の理由にしない。判断軸は**信頼性**である
  - Why: プロセスに求められるのは信頼性である。頼った結果、オーバーヘッドばかりで手戻りが減らず、決めるべきことが漏れるなら、そのプロセスは意味をなさない
- 却下は成果である。**採らないと決めたことは、その旨を明記する**。「書かれていない」と「課さないと決めた」を読み手が区別できる状態を保つ
- 実証を欠く判断は、必ず根拠水準マーク(ADR-0026)で開示する
