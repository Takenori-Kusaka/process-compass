...
## Summary
- 3663 nodes · 3558 edges · 308 communities (302 shown, 6 thin omitted)
...
## Communities (308 total, 6 thin omitted)
...
### Community 296 - "3. アンチパターン各論(本メモの中心)"
Cohesion: 0.29
...
## Knowledge Gaps
- **2767 isolated node(s):** `0.1 信頼度の区分`, `0.2 数値の扱いに関する方針`, `0.3 既存調査メモとの関係（重複回避）`, `1. 前提の確認: 内部矛盾の所在`, `2.1 ベンチマーク: 短期タスクは飽和、長期タスクは低水準のまま` (+2762 more)
...
- **Why does `コンテキストエンジニアリング手法 調査メモ` connect `コンテキストエンジニアリング手法 調査メモ` to `20260710-jp-governance.md`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **What connects `0.1 信頼度の区分`, `0.2 数値の扱いに関する方針`, `0.3 既存調査メモとの関係（重複回避）` to the rest of the system?**
...
- **Should `日本企業のガバナンス・決裁ゲートの実態 調査メモ` be split into smaller, more focused modules?**
